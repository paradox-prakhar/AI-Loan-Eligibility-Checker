...
import type { CreditRating, EligibilityStatus, EmploymentType, LoanType, RiskClass } from './types';
...
const LOAN_MULTIPLIER: Record<LoanType, number> = {
  HOME: 18,
  CAR: 8,
  EDUCATION: 10,
  BUSINESS: 12,
...
const CREDIT_RATING_BY_SCORE = (score: number): CreditRating => {
...
const riskClassFromScore = (score: number): RiskClass => {
...
export function calculateEmi(input: EmiCalculatorInput) {
...
export function analyzeLoanEligibility(input: LoanEligibilityInput) {
  ...
  const riskScore = clamp(
    100 - creditStrength * 0.35 - Math.max(0, 40 - disposableIncome / Math.max(input.monthlyIncome, 1) * 100) * 0.2 - savingsBuffer * 0.15 - employmentStrength * 0.1,
    0,
    100
  );
  const confidenceScore = clamp(100 - Math.abs(riskScore - 50), 55, 98);
  const maxLoanAmount = Math.max(
    input.monthlyIncome * LOAN_MULTIPLIER[input.loanType] * EMPLOYMENT_MULTIPLIER[input.employmentType] * (input.creditScore / 850),
    0
  );
  const status: EligibilityStatus =
    input.creditScore >= 780 && foir <= 55 && dti <= 40
      ? 'ELIGIBLE'
...
export function analyzeCreditProfile(input: CreditAnalysisInput) {
  ...
  const predictedScore = clamp(
    input.creditScore + paymentBonus + ageBonus - utilizationPenalty - input.loans * 2 - input.creditCards,
    300,
    900
  );
...
export function analyzeRiskProfile(input: RiskEngineInput) {
...
export function recommendLoanTypes(input: { monthlyIncome: number; creditScore: number; riskScore: number; eligibleAmount: number }) {
  const recommendations = [
    { type: 'HOME' as const, score: scoreRecommendation(input, 'HOME') },
    { type: 'CAR' as const, score: scoreRecommendation(input, 'CAR') },
    { type: 'EDUCATION' as const, score: scoreRecommendation(input, 'EDUCATION') },
    { type: 'BUSINESS' as const, score: scoreRecommendation(input, 'BUSINESS') },
    { type: 'PERSONAL' as const, score: scoreRecommendation(input, 'PERSONAL') },
    { type: 'GOLD' as const, score: scoreRecommendation(input, 'GOLD') },
  ]
...
function scoreRecommendation(input: { monthlyIncome: number; creditScore: number; riskScore: number; eligibleAmount: number }, loanType: LoanType) {
  const base = input.creditScore / 9 - input.riskScore * 0.4 + Math.min(input.monthlyIncome / 1000, 20);
  const typeWeight: Record<LoanType, number> = {
    HOME: 14,
    CAR: 12,
    EDUCATION: 10,
    BUSINESS: 9,
...
function recommendationReason(loanType: LoanType, input: { monthlyIncome: number; creditScore: number; riskScore: number }) {
  if (loanType === 'HOME') {
...
export function buildAdviceContext(params: {
  eligibility: ReturnType<typeof analyzeLoanEligibility>;
  credit: ReturnType<typeof analyzeCreditProfile>;
  risk: ReturnType<typeof analyzeRiskProfile>;
  goals: string[];
}) {
  return [
